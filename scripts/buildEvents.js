...
function getFunction(func, prefix, type) {
  ...
  // Check for custom beforeBody/afterBody code
  // Use preEvent.beforeBody for pre hooks, postEvent.afterBody for post hooks
  const isPostHook = prefix.startsWith('post');
  const eventKey = isPostHook ? 'postEvent' : 'preEvent';
  const beforeBody = !isPostHook ? (customs[type]?.[func.name]?.preEvent?.beforeBody || '') : '';
  const afterBody = customs[type]?.[func.name]?.[eventKey]?.afterBody || '';
  const beforeBodyCode = beforeBody ? `\n    ${beforeBody}\n` : '';
  const afterBodyCode = afterBody ? `\n    ${afterBody}` : '';

  return `${description}
  ${func.type} ${prefix}_${func.name} (${func.args.map(v => `${v.type} ${v.name}`).join(', ')}) {
    SET_META_RESULT(MRES_IGNORED);${beforeBodyCode}
    event::findAndCall("${eventName}", [=](v8::Isolate* isolate) {
      unsigned int v8_argCount = ${regularArgs.length};
      v8::Local<v8::Value>* v8_args = new v8::Local<v8::Value>[${regularArgs.length}];
      ${regularArgs.map((v, i) => `v8_args[${i}] = ${getFixedArgToValue(v)}; // ${v.name} (${v.type})`).join('\n      ')}
      return std::pair<unsigned int, v8::Local<v8::Value>*>(v8_argCount, v8_args);
    });${afterBodyCode}${returnStatement}
  }`;
}
...
(async () => {
  ...
  // Generate test file that registers all hooks
  const testFile = generateTestFile(dllFunctions, engineFunctions, eventInterfaces);
  await fs.writeFile('./packages/examples/src/examples/test-all-hooks.generated.ts', testFile);
  console.log('Generated test-all-hooks.generated.ts');

})();

function generateTestFile(dllFunctions, engineFunctions, eventInterfaces) {
  // Build a map of event name to interface for quick lookup
  const interfaceMap = new Map();
  for (const iface of eventInterfaces) {
    interfaceMap.set(iface.name, iface);
  }

  function generateHookHandler(eventName, iface) {
    if (!iface || !iface.parameters || iface.parameters.length === 0) {
      return `nodemod.on('${eventName}', () => {
  console.log('[${eventName}]');
});`;
    }

    const params = iface.parameters.map(p => p.name).join(', ');
    const logArgs = iface.parameters.map(p => {
      // For primitive types, we can include in the label
      if (p.type === 'number' || p.type === 'boolean' || p.type === 'string') {
        return `'${p.name}:', ${p.name}`;
      }
      // For objects/arrays, log separately so they expand properly
      return `'${p.name}:', ${p.name}`;
    }).join(', ');

    return `nodemod.on('${eventName}', (${params}) => {
  console.log('[${eventName}]', ${logArgs});
});`;
  }
  ...
  // DLL events
  for (const func of dllFunctions) {
    const eventName = getEventName(func, 'dll');
    const iface = interfaceMap.get(eventName);
    handlers.push(generateHookHandler(eventName, iface));
  }

  // Post-DLL events
  for (const func of dllFunctions) {
    const eventName = getEventName(func, 'postDll');
    const iface = interfaceMap.get(eventName);
    handlers.push(generateHookHandler(eventName, iface));
  }

  // Engine events (skip CRC32)
  for (const func of engineFunctions.filter(v => !v.name.includes('CRC32'))) {
    const eventName = getEventName(func, 'eng');
    const iface = interfaceMap.get(eventName);
    handlers.push(generateHookHandler(eventName, iface));
  }

  // Post-Engine events (skip CRC32)
  for (const func of engineFunctions.filter(v => !v.name.includes('CRC32'))) {
    const eventName = getEventName(func, 'postEng');
    const iface = interfaceMap.get(eventName);
    handlers.push(generateHookHandler(eventName, iface));
  }
...
// Total hooks: ${handlers.length}

${handlers.join('\n\n')}

console.log('test-all-hooks: Registered ${handlers.length} event handlers');
`;
}
...
function parseStructureProperties(content, structureName) {
  ...
  while ((match = manualAccessorRegex.exec(content)) !== null) {
    ...
    if (propName === 'pNext' && contentAfter.includes('wrapCustomization')) {
      ...
      tsType = 'number[]'; // Generic arrays (check BEFORE Number::New since array code has both)
  ...
  while ((match = setCallRegex.exec(content)) !== null) {
    ...
    // Handle ternary expressions with null checks
      ...
      // This is a nullable type - check what wrapper is used (use opening paren to avoid substring matches)
