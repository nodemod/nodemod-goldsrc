...
// Ham hook return values - matches AMX Mod X
enum HamResult {
    HAM_UNSET = 0,      // Default state
    HAM_IGNORED,        // Hook had no effect, continue normally
    HAM_HANDLED,        // Hook processed the call, but still call original
    HAM_OVERRIDE,       // Use hook's return value instead of original
    HAM_SUPERCEDE       // Don't call original function at all
};

// Ham function types - matches AMX Mod X Ham constants exactly
// These are indices into the hooklist array and must match AMX Mod X order
enum HamType {
    // CBaseEntity virtuals
    Ham_Spawn = 0,
    Ham_Precache,
    Ham_Keyvalue,
    Ham_ObjectCaps,
    ...
    // Mod-specific: The Specialists
    Ham_TS_BreakableRespawn,
    Ham_TS_CanUsedThroughWalls,
    Ham_TS_RespawnWait,

    // Mod-specific: Counter-Strike
    Ham_CS_Restart,
    Ham_CS_RoundRespawn,
    Ham_CS_Item_CanDrop,
    Ham_CS_Item_GetMaxSpeed,

    // Mod-specific: Day of Defeat
    ...
    // Mod-specific: Team Fortress Classic
    Ham_TFC_EngineerUse,
    Ham_TFC_Finished,
    Ham_TFC_EmpExplode,
    ...
    // CBaseMonster virtuals (2011 additions)
    Ham_ChangeYaw,
    Ham_HasHumanGibs,
    Ham_HasAlienGibs, 
    Ham_FadeMonster,
    ...
    // More CBaseMonster virtuals (not in CS/NS/TS)
    Ham_RunAI,
    Ham_MonsterThink,
    Ham_MonsterInit,
    Ham_CheckLocalMove,
    ...
    Ham_EndMarker  // Always last
};

// Error codes
enum HamError {
    HAM_OK = 0,
    HAM_INVALID_FUNC,           // Function doesn't exist
    HAM_FUNC_NOT_CONFIGURED,    // Function not in gamedata
    HAM_FUNC_NOT_AVAILABLE,     // Function not supported in this mod
    HAM_INVALID_ENTITY,         // Entity is invalid
    HAM_HOOK_EXISTS             // Hook already registered
};
...
enum HamParamType {
    ...
    HAM_PARAM_STRING,
    HAM_PARAM_TRACE,
    HAM_PARAM_EDICT,    // Pointer to edict_t (similar to entity but raw edict)
    HAM_PARAM_ITEMINFO  // Pointer to ItemInfo struct
};
