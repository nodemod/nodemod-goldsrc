...
void createEntvarsTemplate(v8::Isolate* isolate) {
    ...
    // Fields in exact order as they appear in progdefs.h
    ...
    // Array fields (controller and blending) with getter/setter support
    templ->SetNativeDataProperty(v8::String::NewFromUtf8(isolate, "controller").ToLocalChecked(),
        [](v8::Local<v8::Name> property, const v8::PropertyCallbackInfo<v8::Value> &info) {
            ...
            for (int i = 0; i < 4; i++) {
                arr->Set(context, i, v8::Number::New(info.GetIsolate(), entvars->controller[i])).Check();
            }
            info.GetReturnValue().Set(arr);
        },
        [](v8::Local<v8::Name> property, v8::Local<v8::Value> value, const v8::PropertyCallbackInfo<void> &info) {
            entvars_t *entvars = unwrapEntvars_internal(info.GetIsolate(), info.Holder());
            if (entvars == nullptr || !value->IsArray()) return;
            ...
            for (uint32_t i = 0; i < 4 && i < length; i++) {
                ...
                if (val->IsNumber()) {
                    entvars->controller[i] = SETBYTE(val);
                }
            }
        });
    ...
    templ->SetNativeDataProperty(v8::String::NewFromUtf8(isolate, "blending").ToLocalChecked(),
        [](v8::Local<v8::Name> property, const v8::PropertyCallbackInfo<v8::Value> &info) {
            ...
            v8::Local<v8::Array> arr = v8::Array::New(info.GetIsolate(), 2);
            auto context = info.GetIsolate()->GetCurrentContext();
            for (int i = 0; i < 2; i++) {
                arr->Set(context, i, v8::Number::New(info.GetIsolate(), entvars->blending[i])).Check();
            }
            info.GetReturnValue().Set(arr);
        },
        [](v8::Local<v8::Name> property, v8::Local<v8::Value> value, const v8::PropertyCallbackInfo<void> &info) {
            ...
            for (uint32_t i = 0; i < 2 && i < length; i++) {
                v8::Local<v8::Value> val = arr->Get(context, i).ToLocalChecked();
                if (val->IsNumber()) {
                    entvars->blending[i] = SETBYTE(val);
                }
            }
        });
